...
import decimal
from dashboard.models import Performance, DnaResult, PerformanceAnalyse, PerformanceBenchmark
...
def post_performance_result_handler(sender, instance=None, created=False, **kwargs):
    ...
    if instance.measurement.slug_name == 'height':
        if DnaResult.objects.filter(dna_measurement=instance.measurement.related_dna_measurement,
            ...
            if bio_age and slope:
                PerformanceAnalyse.objects.create(player=instance.player,
                                                  bio_age=bio_age,
    ...
    PerformanceBenchmark.objects.create(performance=instance, benchmark=benchmark, benchmark_bio=benchmark_bio)
